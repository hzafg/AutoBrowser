...
{
    WebPage *webPage = dynamic_cast<WebPage*>(webView->page());
    webPage->setParentTab(this);
...
void TabWidget::closeTab(int index)
{
    if (WebView *view = webView(index)) {
        std::cout << "close tab " << view->url().toString().toUtf8().constData() << std::endl;
        ...
        ScriptLoader::runScript(
                    dynamic_cast<WebPage*>(
                        this->currentWebView()->page()),
                    ScriptLoader::tabCloseScript, view->url().toString().toUtf8().constData());
    }
}
